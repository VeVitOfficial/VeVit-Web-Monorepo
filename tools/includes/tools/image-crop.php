<div class="stack" style="max-width:50rem;margin:0 auto">
  ...
  <div class="hidden" id="cr-work">
    ...
    <fieldset class="crop-fields"><legend class="sr-only"><?= e(vv_t('image_crop.hint', $lang)) ?></legend>
      <?php foreach (['x' => 'X', 'y' => 'Y', 'w' => 'W', 'h' => 'H'] as $id => $label): ?><label><span><?= $label ?></span><input class="input" id="cr-<?= $id ?>" type="number" min="0" step="1" inputmode="numeric"></label><?php endforeach; ?>
    </fieldset>
    <div class="row" style="flex-wrap:wrap;gap:0.5rem;margin-top:0.75rem;align-items:end">
      ...
      <button class="btn btn-primary btn-touch" id="cr-apply" type="button" disabled><?= icon_svg('Crop', 16) ?> <?= e(vv_t('image_crop.apply', $lang)) ?></button>
    </div>
  </div>
  <div class="before-after hidden" id="cr-result-preview"><div class="ba-pane"><span class="ba-label"><?= e(vv_t('tool_common.result', $lang)) ?></span><div class="ba-frame"><img id="cr-result-image" alt=""></div></div></div>
