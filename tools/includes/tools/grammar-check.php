<div class="stack" style="max-width:46rem;margin:0 auto">
  ...
  <textarea class="textarea" id="gc-input" rows="10" placeholder="<?= e(vv_t('grammar_check.input_ph', $lang)) ?>"></textarea>
  <button class="btn btn-primary btn-touch" id="gc-run" type="button"><?= icon_svg('SpellCheck', 18) ?> <span class="gc-label"><?= e(vv_t('grammar_check.fix', $lang)) ?></span><span class="gc-stop hidden"><?= e(vv_t('tool_common.stop', $lang)) ?></span></button>
  ...
  <div class="result-card hidden" id="gc-out">
    ...
    <div class="row" style="gap:0.5rem;margin-top:0.75rem"><button class="btn btn-secondary btn-sm" id="gc-copy" type="button"><?= icon_svg('Copy', 14) ?> <?= e(vv_t('tool_common.copy', $lang)) ?></button></div>
  </div>
  <p class="muted" style="font-size:0.8rem"><?= e(vv_t('grammar_check.footer', $lang)) ?></p>
</div>
