...
$first = (string)file_get_contents(HUB_ROOT . '/tools/assets/data/tools.json');
$firstIndex = (string)file_get_contents(HUB_ROOT . '/tools/assets/data/tools-index.json');
hub_assert(hub_command([PHP_BINARY, 'scripts/export-tools.php'], $output) === 0, 'second export failed: ' . $output);
hub_assert($first === (string)file_get_contents(HUB_ROOT . '/tools/assets/data/tools.json'), 'tools.json is not deterministic');
hub_assert($firstIndex === (string)file_get_contents(HUB_ROOT . '/tools/assets/data/tools-index.json'), 'tools-index.json is not deterministic');
hub_assert(hub_command([PHP_BINARY, 'scripts/export-tools.php', '--check'], $output) === 0, '--check failed: ' . $output);
$dataset = hub_json(HUB_ROOT . '/tools/assets/data/tools.json');
$generatorDataset = hub_json(HUB_ROOT . '/tools/assets/data/tools-index.json');
$tools = all_tools();
