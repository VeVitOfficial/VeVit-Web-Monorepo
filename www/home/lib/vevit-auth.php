...
function vevitAuthNormalizeAbsolutePath(string $path): ?string
{
...
function vevitAuthConfigPathAllowedByOpenBasedir(string $path, ?string $openBasedir = null): bool
{
    $normalizedPath = vevitAuthNormalizeAbsolutePath($path);
    ...
    foreach (explode(PATH_SEPARATOR, $openBasedir) as $allowedPath) {
        $normalizedAllowedPath = vevitAuthNormalizeAbsolutePath($allowedPath);
        if ($normalizedAllowedPath === null) continue;
...
/** @return list<string> */
function vevitAuthConfigCandidates(?string $configuredPath = null): array
{
    $configuredPath = $configuredPath ?? getenv('VEVIT_HOME_CONFIG_PATH');
    $paths = [];
...
function vevitAuthFindConfigPath(?string $configuredPath = null, ?string $openBasedir = null): ?string
{
    foreach (vevitAuthConfigCandidates($configuredPath) as $candidate) {
        if (
            !vevitAuthConfigPathAllowedByOpenBasedir($candidate, $openBasedir)
            || !is_file($candidate)
