...
expect_true(
    preg_match('/<a class="dd-item" href="\/account" role="menuitem">[\s\S]*?data-ui-text="landing\.account\.navigation"[\s\S]*?<\/a>/u', $html) === 1,
    'Account must be an active destination in the desktop applications menu.'
);
expect_true(
    preg_match('/<a class="ec ec-account" href="\/account"[^>]*>[\s\S]*?data-ui-text="landing\.account\.card"[\s\S]*?<\/a>/u', $html) === 1,
...
foreach ($latestTools as $index => [$slug, $name, $category, $recommended]) {
    ...
    expect_true(
        str_contains($card, 'href="/tools/' . $slug . '"')
