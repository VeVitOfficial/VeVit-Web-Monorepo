...
?>
<!doctype html><html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><title>Dvoufázové ověření · VEVIT</title><link rel="icon" href="/account/images/icon.ico" type="image/x-icon">
...
<?php if(!$valid):?><h1>Ověření vypršelo</h1><p>Vraťte se na přihlášení a zkuste to znovu.</p><a href="/account/login">Zpět na přihlášení</a>
<?php else:?><h1>Dvoufázové ověření</h1><p id="description">Zadejte šestimístný kód z ověřovací aplikace.</p><form id="form" data-challenge="<?= htmlspecialchars($challenge, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>"><label for="code">Ověřovací kód</label><input id="code" inputmode="numeric" autocomplete="one-time-code" maxlength="14" required><button class="btn primary" id="submit" type="submit">Ověřit a přihlásit</button></form><button class="btn ghost" id="switch" type="button">Použít obnovovací kód</button><p class="status" id="status" role="alert" aria-live="polite"></p><a href="/account/login">Zpět na přihlášení</a>
<script src="/account/assets/verify-2fa.js"></script>
