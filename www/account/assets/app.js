...
async function loadPreferences(force = false) {
  try { const data = !force && sectionCache.has('preferences') ? sectionCache.get('preferences') : await api('preferences.php'); sectionCache.set('preferences', data); const p = data.preferences || {}; const language = currentUser?.language || 'cs'; const languages = [['cs','Čeština'],['en','English'],['de','Deutsch'],['es','Español'],['uk','Українська'],['fr','Français'],['sk','Slovenčina']]; const zones = [['Europe/Prague','Praha'],['Europe/Bratislava','Bratislava'],['Europe/Berlin','Berlín'],['Europe/Madrid','Madrid'],['Europe/Paris','Paříž'],['Europe/Kyiv','Kyjev'],['Europe/London','Londýn'],['America/New_York','New York'],['America/Los_Angeles','Los Angeles'],['Asia/Tokyo','Tokio'],['Australia/Sydney','Sydney'],['UTC','UTC']]; const languageOptions = languages.map(([value,label]) => `<option value="${value}" ${language === value ? 'selected' : ''}>${label}</option>`).join(''); const timezoneOptions = zones.map(([value,label]) => `<option value="${value}" ${p.timezone === value ? 'selected' : ''}>${label} (${value})</option>`).join(''); setSectionHtml('preferencesState', `<form class="card settings-form" id="preferencesForm"><div class="card-heading"><div><h2>Jazyk a region</h2><p>Vybraný jazyk je výchozí napříč VeVit. V hlavičce pak vždy rychle přepnete mezi ním a angličtinou.</p></div></div><div class="form-grid"><div class="form-field"><label for="prefLanguage">Výchozí jazyk</label><select class="input" id="prefLanguage" name="language">${languageOptions}</select></div><div class="form-field"><label for="prefTimezone">Časové pásmo</label><select class="input" id="prefTimezone" name="timezone">${timezoneOptions}</select></div><div class="form-field"><label for="prefDateFormat">Formát data</label><select class="input" id="prefDateFormat" name="date_format"><option ${p.date_format === 'DD. MM. YYYY' ? 'selected' : ''}>DD. MM. YYYY</option><option ${p.date_format === 'YYYY-MM-DD' ? 'selected' : ''}>YYYY-MM-DD</option></select></div><div class="form-field"><label for="prefWeek">První den týdne</label><select class="input" id="prefWeek" name="week_starts_on"><option value="monday" ${p.week_starts_on === 'monday' ? 'selected' : ''}>Pondělí</option><option value="sunday" ${p.week_starts_on === 'sunday' ? 'selected' : ''}>Neděle</option></select></div></div><div class="form-actions"><button class="btn btn--primary" type="submit">Uložit nastavení</button></div></form>`); } catch (error) { sectionError('preferencesState', 'Nastavení se nepodařilo načíst.', 'preferences'); }
}
...
async function handleSettingsSubmit(event) {
  ...
  try {
    ...
    await api(endpoint, { method:'POST', body }); if (form.id === 'preferencesForm' && form.elements.language.value !== currentUser.language) { const result = await api('profile-update.php', { method:'PATCH', body:{ language:form.elements.language.value } }); currentUser = result.user; window.dispatchEvent(new CustomEvent('vevit:locale-preferencechange', { detail:{ locale:currentUser.language } })); } sectionCache.delete(key); showToast('Nastavení byla uložena.');
