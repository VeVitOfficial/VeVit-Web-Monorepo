// Port konstant a konfigurace z edu/js/config.js.
// Totály programování, barvy obtížnosti, kategorie a pomocné počítátka kurzů.
...
export type DifficultyKey = keyof typeof DIFFICULTY_COLOR;

// Kategorie → ikona + barva pro dashboard
...
// Typy pro kurz a lekci (odpovídají tvaru JSON v public/edu/data/courses)
...
export interface CourseIndexMeta {
  id: string;
  slug: string;
  title: string;
  description: string;
...
export interface Course extends Omit<CourseIndexMeta, "chapters" | "lessons"> {
  chapters: Chapter[];
}
...
export function courseXP(course: Course): number {
...
export function levelFromCompleted(completed: number): number {
