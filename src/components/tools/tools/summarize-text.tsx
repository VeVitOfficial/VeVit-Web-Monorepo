...
const STYLES = ["odrážky", "krátký odstavec", "jedna věta"] as const;
const STYLE_LABELS: Record<string, string> = {
  "odrážky": "Odrážky (max 6)",
  "krátký odstavec": "Krátký odstavec",
  "jedna věta": "Jedna věta",
};

export default function SummarizeText({ locale }: ToolComponentProps) {
  void locale;
  const [input, setInput] = useState("");
  const [style, setStyle] = useState<string>("odrážky");
  ...
  const go = useCallback(() => {
    ...
    setError("");
    setHasOut(true);
    if (mdRef.current) mdRef.current.textContent = "Shrnuji…";
    setRunning(true);
    const prompt = "Shrň následující text. Výstup ve formě: " + style + ".\n\n" + text;
  ...
  return (
    <div className="stack" style={{ maxWidth: "46rem", margin: "0 auto" }}>
      ...
      <div className="stack-sm"><label className="field-label" htmlFor="ts-style">Styl shrnutí</label>
        <select className="select" id="ts-style" value={style} onChange={(e) => setStyle(e.target.value)}>
          {STYLES.map((s) => <option key={s} value={s}>{STYLE_LABELS[s]}</option>)}
        </select></div>
      <textarea className="textarea" id="ts-input" rows={10} placeholder="Vložte delší text ke shrnutí…" value={input} onChange={(e) => setInput(e.target.value)} />
      <button className="btn btn-primary btn-touch" id="ts-run" type="button" disabled={!depsReady && !depsFailed} onClick={go}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 5H3" /><path d="M15 12H3" /><path d="M17 19H3" /></svg>
        <span className="ts-label" style={{ display: running ? "none" : undefined }}>Shrnout</span>
        <span className="ts-stop" style={{ display: running ? undefined : "none" }}>Zastavit</span>
      </button>
      ...
      <div className="ai-error hidden" id="ts-error" style={{ display: error ? undefined : "none" }}>
        ...
        <span id="ts-error-text">{error}</span>
      </div>
      {hasOut ? (
        <div className="result-card" id="ts-out">
          <div className="markdown-body" id="ts-md" ref={mdRef} />
          <div className="row" style={{ gap: "0.5rem", marginTop: "0.75rem" }}>
            <button className="btn btn-secondary btn-sm" id="ts-copy" type="button" onClick={onCopy}><Icon name="Copy" size={14} /> Kopírovat</button>
          </div>
        </div>
      ) : null}
      <p className="muted" style={{ fontSize: "0.8rem" }}>Shrnutí běží lokálně přes Ollamu. Vstup max 20 000 znaků.</p>
    </div>
  );
}