...
// AI SEO meta generátor — React port legacy tools/assets/js/tools/ai-seo.js.
...
function runAi(opts: AiOpts): { abort: () => void } {
  ...
  fetch("/tools/api/ai/ollama.php", {
    ...
    body: JSON.stringify({ prompt: opts.prompt, tool: "ai-seo", stream: true }),
...
const TXT = {
  ...
  run: "Vygenerovat meta",
  stop: "Zastavit",
  copy: "Kopírovat",
  errEmpty: "Vložte obsah nebo klíčová slova.",
  working: "Generuji…",
  note: "Vygeneruje SEO title (≤60 znaků) a meta description (≤155 znaků). Běží lokálně přes Ollamu. Vstup max 20 000 znaků.",
  placeholder: "Vložte obsah stránky (nebo klíčová slova)…",
};

export default function AiSeo({ locale }: ToolComponentProps) {
  ...
  return (
    <div className="stack" style={{ maxWidth: "46rem", margin: "0 auto" }}>
      ...
      <textarea className="textarea" id="seo-input" rows={10} placeholder={TXT.placeholder} value={input} onChange={(e) => setInput(e.target.value)} />
      <button className="btn btn-primary btn-touch" id="seo-run" type="button" onClick={go}>
        ...
        <span className="seo-label" style={{ display: running ? "none" : undefined }}>{TXT.run}</span>
        <span className="seo-stop" style={{ display: running ? undefined : "none" }}>{TXT.stop}</span>
      </button>
      <div className={`ai-error${error ? "" : " hidden"}`} id="seo-error">
        ...
        <span id="seo-error-text">{error}</span>
      </div>
      <div className={`result-card${hasOut ? "" : " hidden"}`} id="seo-out">
        <div className="markdown-body" id="seo-md" ref={mdRef} />
        <div className="row" style={{ gap: "0.5rem", marginTop: "0.75rem" }}>
          <button className="btn btn-secondary btn-sm" id="seo-copy" type="button" onClick={() => { const el = mdRef.current; void copyText(el?.innerText || el?.textContent || "", locale); }}>
