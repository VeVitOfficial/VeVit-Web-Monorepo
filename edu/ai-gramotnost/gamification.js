...
const LocalProgress = {
  ...
  checkAchievements() {
    const d = this.load(); const have = new Set(d.achievements); const newly = [];
    ACHIEVEMENTS.forEach(a => {
      if (have.has(a.id)) return;
      if (this.met(a)) { have.add(a.id); d.totalXp += a.xp_reward; newly.push(a); }
    });
    d.achievements = [...have];
    newly.forEach(showAchievementBanner);
  },
  met(a) {
...
function showXpToast(amount, reason) {
  const c = document.getElementById("xp-toasts"); if (!c || !amount) return;
  const t = el("div", { class: "xp-toast" });
  t.innerHTML = `<span class="ic">${icon("zap", "icon")}</span><div><div class="amt">+${formatXP(amount)} XP</div>${reason ? `<div style="font-size:.78rem;color:var(--text-secondary)">${escapeHTML(reason)}</div>` : ""}</div>`;
  c.appendChild(t);
...
function showAchievementBanner(ach) {
  ...
  const wrap = el("div", {});
  wrap.innerHTML = `<div class="ach-banner"><div class="ic">${icon(ach.icon || "trophy", "icon")}</div><div><div style="font-weight:700">Achievement odemčen!</div><div style="color:var(--text-secondary);font-size:.85rem">${escapeHTML(ach.name)} • +${ach.xp_reward} XP</div></div></div>`;
  c.appendChild(wrap.firstChild);
  setTimeout(() => { const b = c.querySelector(".ach-banner"); if (b) { b.style.transition = "opacity .4s"; b.style.opacity = "0"; setTimeout(() => b.remove(), 400); } }, 3500);
}
