...
function oauth_callback_run(string $provider): never {
  ...
  if (is_string($initiatedUserId) && $initiatedUserId !== '') {
    ...
    if (str_starts_with((string)($attempt['destination'] ?? ''),'twofa_reauth:')) {
      ...
      $reauthLang = vv_resolve_locale(); vv_locale_cookie($reauthLang);
      header('Cache-Control: no-store');header('Location: ' . vv_locale_redirect_target('/account/security', $reauthLang) . '?twofa_reauth='.rawurlencode($reauthId).'&twofa_action='.rawurlencode($twofaAction),true,302);exit;
    }
    ...
    $connectLang = vv_resolve_locale(); vv_locale_cookie($connectLang);
    header('Cache-Control: no-store'); header('Location: ' . vv_locale_redirect_target('/account/connections', $connectLang), true, 302); exit;
  }
  ...
  $user = _auth_find_one($cfg, 'users', ['id' => $userId], 'id,email,full_name,nickname,language');
  if (!is_array($user['data'] ?? null)) oauth_redirect_to_login('oauth_profile_failed');
  $oauthLang = vv_locale_valid($user['data']['language'] ?? 'cs');
  if (totpIsActive($cfg, $userId)) {
    ...
    vv_locale_cookie($oauthLang);
    header('Cache-Control: no-store');
    header('Location: ' . vv_locale_redirect_target('/account/verify-2fa.php', $oauthLang) . '?challenge=' . rawurlencode($challenge), true, 302);
    exit;
  }
  ...
  header('Cache-Control: no-store');
  header('Location: ' . vv_locale_redirect_target($destination, $oauthLang), true, 302);
  exit;
}
