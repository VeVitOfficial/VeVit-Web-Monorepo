 ...
 */
function auth_normalize_absolute_path(string $path): ?string {
...
/**
 * Avoid filesystem calls that would emit an open_basedir warning.
 */
function auth_config_path_allowed_by_open_basedir(string $path, ?string $openBasedir = null): bool {
  $normalizedPath = auth_normalize_absolute_path($path);
  ...
  foreach (explode(PATH_SEPARATOR, $openBasedir) as $allowedPath) {
    $normalizedAllowedPath = auth_normalize_absolute_path($allowedPath);
    if ($normalizedAllowedPath === null) {
      continue;
    }
    $prefix = rtrim($normalizedAllowedPath, '/');
    if ($prefix === '') {
      return true;
    }
...
/** @return list<string> */
function auth_config_candidates(?string $configuredPath = null): array {
  $configuredPath = $configuredPath ?? getenv('VEVIT_ACCOUNT_CONFIG_PATH');
  $paths = [];
  if (is_string($configuredPath) && $configuredPath !== '') {
    $paths[] = $configuredPath;
  }
  // WEDOS: sourozenec document rootu, stále uvnitř open_basedir.
  $paths[] = dirname(__DIR__, 3) . '/vevit-private/account.php';
  return $paths;
}

function auth_find_config_path(?string $configuredPath = null, ?string $openBasedir = null): ?string {
  foreach (auth_config_candidates($configuredPath) as $candidate) {
    if (
      !auth_config_path_allowed_by_open_basedir($candidate, $openBasedir)
      || !is_file($candidate)
...
function auth_config_failure(): never {
  if (!headers_sent()) {
...
function auth_load_config(): array {
  // Produkční warningy nesmí odhalovat cesty ani zneplatnit HTTP hlavičky.
  @ini_set('display_errors', '0');

  $path = auth_find_config_path();
  if ($path === null) {
    auth_config_failure();
  }
...
function _auth_filtered_get(
  ...
  if ($adapter !== null) {
    $result = $adapter($cfg, $table, $filters, $select, $limit, $orderBy);
    return is_array($result) ? $result : ['error' => 'Invalid lookup result'];
  }
  ...
  if ($orderBy !== null) {
    $query[] = 'order=' . rawurlencode($orderBy);
  }
  if ($limit !== null) {
...
/**
 * Build the only browser-facing authenticated-session payload.
 * The CSRF token remains derived from the HttpOnly cookie and server secret;
 * the raw session token is never included in this response.
 */
function me_response_payload(array $cfg, array $user): array {
  $rawToken = $_COOKIE[VV_COOKIE] ?? $_COOKIE[VV_COOKIE_LEG] ?? null;
  if (!_vv_valid_token_format($rawToken)) {
    throw new LogicException('Authenticated response requires a valid session token');
  }

  return [
    'authenticated' => true,
    'user' => $user,
    'csrf_token' => vv_csrf_token($rawToken, $cfg),
  ];
}
...
function createSession(array $cfg, string $userId, bool $remember = false): string {
  ...
  $result = _auth_insert($cfg, 'sessions', [
    // Produkční schema už neobsahuje plaintext session_token. Token zůstává
    // výhradně v HttpOnly cookie; databáze dostává jen jeho SHA-256 hash.
  ...
  if (
    !is_array($inserted)
    || !isset($inserted['token_hash'])
    || !is_string($inserted['token_hash'])
    || !hash_equals($tokenHash, $inserted['token_hash'])
  ) {
  ...
  if (
    ...
    _auth_delete($cfg, 'sessions', ['token_hash' => $tokenHash]);
    jsonErr('Unable to create session', 500);
  }
...
function renewSession(array $cfg, string $token, bool $remember): void {
  $now = time();
  _auth_update($cfg, 'sessions', ['token_hash' => hash('sha256', $token)], [
    'last_seen_at' => gmdate('Y-m-d\TH:i:s\Z', $now),
  ]);
}
...
function destroySession(array $cfg): void {
  ...
  foreach ([$newToken, $legToken] as $tok) {
    ...
    // Tabulka relací obsahuje pouze SHA-256 hash tokenu.
    $result = _auth_update($cfg, 'sessions', ['token_hash' => hash('sha256', $tok)], [
 ...
 * $rawBody overrides reading php://input — used by isolated endpoint tests.
 */
function jsonBody(?string $rawBody = null): array {
