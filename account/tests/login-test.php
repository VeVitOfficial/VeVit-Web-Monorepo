...
/**
 * Build a login request body that passes the honeypot/timing anti-bot gate
 * by default, so scenarios only need to spell out the fields they actually
 * care about testing.
 */
function login_test_body(array $fields): string {
  $fields += [
    'hp_confirm' => '',
...
function run_login_child(string $scenario): never {
  ...
  switch ($scenario) {
    ...
    case 'empty':
      ...
      _login_run([], login_test_body(['email' => '  ', 'password' => '  ']));
      break;
    ...
    case 'lookup-error':
      ...
      _login_run([], login_test_body(['email' => ' USER@Example.Test ', 'password' => 'secret']));
      break;
    ...
    case 'bad-password':
      ...
      _login_run(
        [],
        login_test_body(['email' => 'user@example.test', 'password' => 'wrong-password'])
      );
      break;
    ...
    case 'invalid-remember':
      ...
      _login_run([], login_test_body(['email' => 'user@example.test', 'password' => 'secret', 'remember' => 'true']));
      break;
    ...
    case 'twofa':
      $GLOBALS['_AUTH_HELPER_ADAPTERS'] = [
        // Discriminate by table, not call order: identifier-backoff adds an
        // extra login_attempts lookup ahead of totpIsActive()'s own query.
        'filtered_get' => static function (array $cfg, string $table): array {
          if ($table === 'user_totp_methods') {
...
login_test('lookup errors stay generic and use the credential projection', function (): void {
  ...
  $lookup = null;
  foreach ($result['meta']['events'] ?? [] as $event) {
    if (($event[0] ?? null) === 'find-one') {
      $lookup = $event;
      break;
    }
  }
